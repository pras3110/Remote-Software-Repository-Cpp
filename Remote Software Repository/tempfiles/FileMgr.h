...
namespace FileManager
{
  ...
  public:
    ...
    {
      std::string fpath = FileSystem1::Path::getFullFileSpec(path);
      dir(fpath);
      for (auto patt : patterns_)
      {
        std::vector<std::string> files = FileSystem1::Directory::getFiles(fpath, patt);
        for (auto f : files)
        {
          file(f);
        }
      }
      std::vector<std::string> dirs = FileSystem1::Directory::getDirectories(fpath);
      for (auto d : dirs)
      {
        if (d == "." || d == "..")
          continue;
