...
const std::string saveFilePathForServer = "../ServerRepository";
const std::string saveFilePathForClient = "../../../../LocalClientRepository";
const std::string sendFilePathForServer = "../SendFilesRepo";
const std::string sendFilePathForClient = "../../../../SendFilesClient";
...
Message Receiver::getMessage()
{
 ...
 // std::cout << "in get message comm.cpp receiver";
  return rcvQ.deQ();
}
  ...
  std::function <void()> threadProc = [&]() {
    while (true)
    {
	 ...
	 // std::cout << "dequeud a message\n";
      ...
      if (msg.to().address != lastEP.address || msg.to().port != lastEP.port || msg.command()=="readytoconnect")
      {
        connecter.shutDown();
        //connecter.close();
...
bool Sender::sendFile(Message msg)
{
  ...
  if (msg.value("source") == "repository") {
	  sendFilePath = sendFilePathForServer;
  }
  if (msg.value("source") == "client") {
	  sendFilePath = sendFilePathForClient;
  }
...
public:
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	size_t indexoffile = fileName.find_last_of("/");
	std::string filename_ = fileName.substr(indexoffile + 1, fileName.size());
	if (msg.value("destination") == "repository") {
		saveFilePath = saveFilePathForServer;
	}
	if (msg.value("destination") == "client") {
		saveFilePath = saveFilePathForClient;
	}
    std::string fileSpec = saveFilePath + "/" + filename_;
...
void Comm::stop()
{
	//std::cout << "Stopping Comm......" << std::endl;
  rcvr.stop();
  sndr.stop();
}
...
void DemoSndrRcvr(const std::string& machineName)
{
  ...
  BlockingQueue<Message>* pQ1 = rcvr1.queue(); ClientHandler ch1(pQ1);
  rcvr1.start(ch1);
  EndPoint ep2;
  ep2.port = 9092;ep2.address = "localhost";
  Receiver rcvr2(ep2);
  ...
  Message msg;
  msg.name("msg #1");msg.to(ep1);
  msg.from(msg.to());msg.command("do it");msg.attribute("bodyAttrib", "zzz");
